...
import numpy as np
from kernel import CauchyKernel,MultiCauchyKernel,MultiMaternKernel
...
#        S = A_hat
        ...
        lambda_mat = K_nm[:, :, None] * K_nm[:, None, :]  # (b, m, m)
        lambda_mat = torch.einsum('ij,bjk,kl->bil', K_mm_inv, lambda_mat, K_mm_inv)
        trace_terms = precision * torch.einsum('bii->b', torch.matmul(A_hat, lambda_mat))
...
class SVGPMK(nn.Module):
    def __init__(self, fixed_inducing_points, initial_inducing_points, dim, fixed_gp_params, kernel_scale, jitter, N_train, dtype, device):
        ...
        # length scale of the kernel
        self.kernel = MultiCauchyKernel(scale=kernel_scale, fixed_scale=fixed_gp_params, dim=dim, dtype=dtype, device=device).to(device)
